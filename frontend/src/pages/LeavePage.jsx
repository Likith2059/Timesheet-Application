...
const StatusBadge = ({ status }) => {
  const cls = { pending: 'badge-pending', approved: 'badge-approved', rejected: 'badge-rejected', cancelled: 'bg-gray-100 text-gray-600 inline-flex px-2 py-0.5 text-xs font-medium rounded-full' };
...
export default function LeavePage() {
  ...
  const [form, setForm] = useState({
    leaveType: 'annual', startDate: '', endDate: '', reason: '', isHalfDay: false, halfDayType: 'morning'
  });
  ...
  const calcDays = () => {
    ...
    try {
      const days = differenceInBusinessDays(parseISO(form.endDate), parseISO(form.startDate)) + 1;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {['annual', 'sick', 'casual', 'unpaid'].map(type => (
          <div key={type} className="card p-4 text-center">
            <p className="text-2xl font-bold text-blue-600">{user?.leaveBalance?.[type] ?? 0}</p>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 font-semibold text-gray-700">Leave History</div>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr key={l._id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 capitalize font-medium text-gray-800">{l.leaveType}</td>
                  ...
                  <td className="px-4 py-3 text-gray-400 text-xs">{format(new Date(l.createdAt), 'MMM d')}</td>
                  <td className="px-4 py-3">
                    {l.status === 'pending' && (
                      <button onClick={() => cancel(l._id)} className="text-xs text-red-600 hover:underline">Cancel</button>
                    )}
                  </td>
                </tr>
              ))}
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" className="input" value={form.startDate} min={format(new Date(), 'yyyy-MM-dd')}
                    onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">To</label>
                  <input type="date" className="input" value={form.endDate} min={form.startDate || format(new Date(), 'yyyy-MM-dd')}
              ...
              {form.isHalfDay && (
                <select className="input" value={form.halfDayType} onChange={e => setForm(f => ({ ...f, halfDayType: e.target.value }))}>
              ...
              {calcDays() > 0 && (
                <div className="bg-blue-50 rounded-lg p-3 text-sm text-blue-700">
                  📅 <strong>{calcDays()} working day{calcDays() > 1 ? 's' : ''}</strong> will be deducted from your {form.leaveType} leave balance.
                </div>
              )}
