...
exports.attendanceReport = async (req, res) => {
  try {
    ...
    const records = await Timesheet.find(filter).sort({ date: 1 }).populate('employee', 'firstName lastName employeeId department designation');
    ...
    if (format === 'pdf') {
      ...
      data.forEach((row, idx) => {
        ...
        x = 40; y = doc.y;
        if (idx % 2 === 0) doc.rect(40, y-2, 760, 14).fill('#f5f5f5').fillColor('black');
        [row.employeeId, row.employeeName, row.department, row.date, row.clockIn, row.clockOut, String(row.totalWorkHours), String(row.breakMinutes)+'m', row.status, row.isLate]
          .forEach((v, i) => { doc.text(v, x, y, { width: cols[i] }); x += cols[i]; });
        doc.moveDown(0.6);
      });
      doc.end();
      return;
    ...
    const summary = {
      month, totalRecords: data.length,
      totalPresent:  data.filter(d => ['present','late'].includes(d.status)).length,
...
exports.dashboardSummary = async (req, res) => {
  try {
    ...
    const [totalEmployees, pendingLeaves, monthRecords] = await Promise.all([
      User.countDocuments({ isActive: true, role: { $ne: 'admin' } }),
      Leave.countDocuments({ status: 'pending' }),
      Timesheet.find({ date: { $gte: monthStart, $lte: monthEnd } })
    ]);

    const presentToday  = await Timesheet.countDocuments({ date: todayDate, status: { $in: ['present','late'] } });
    const activeToday   = await Timesheet.countDocuments({ date: todayDate, clockIn: { $exists: true }, clockOut: { $exists: false } });
    const avgWorkHours  = monthRecords.length ? (monthRecords.reduce((s,r) => s + (r.totalWorkHours||0), 0) / monthRecords.length).toFixed(2) : 0;
    ...
    res.json({
      today: { date: todayDate, totalEmployees, presentToday, absentToday: totalEmployees - presentToday, activeToday, pendingLeaves },
      month: { avgWorkHours: +avgWorkHours, totalOvertimeHours: +(totalOvertime/60).toFixed(2), lateCount: lateThisMonth },
