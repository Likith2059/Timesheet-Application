...
export default function AdminEmployees() {
  ...
  const [modal,   setModal]       = useState(null); // 'add' | { employee }
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await adminAPI.getEmployees({ search, limit: 50 });
  ...
  const openAdd  = () => { setForm({ role: 'employee', department: '', designation: '' }); setModal('add'); };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm">Add, edit, and manage employees</p>
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          <p className="text-gray-400 col-span-3 text-center py-10">Loading...</p>
        ) : employees.map(emp => (
          <div key={emp._id} className="card p-5">
            ...
            <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
              <button onClick={() => openEdit(emp)} className="btn-secondary flex-1 py-1.5 text-xs flex items-center justify-center gap-1">
                <Edit2 className="w-3 h-3" /> Edit
              </button>
              <button onClick={() => resetPwd(emp._id)} className="btn-secondary py-1.5 px-2 text-xs flex items-center gap-1">
                <Key className="w-3 h-3" />
              </button>
              <button onClick={() => deactivate(emp._id)} className="py-1.5 px-2 text-xs border border-red-200 text-red-600 rounded-lg hover:bg-red-50 flex items-center gap-1">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input type="email" className="input" value={form.email || ''} onChange={f('email')} required disabled={modal !== 'add'} />
              </div>
              {modal === 'add' && (
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input className="input" value={form.designation || ''} onChange={f('designation')} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={submitting} className="btn-primary flex-1">
                  {submitting ? 'Saving...' : modal === 'add' ? 'Add Employee' : 'Save Changes'}
                </button>
              </div>
            </form>
          </div>
